...
static int cklink_execute_scan(struct cklink_ctx *ck, struct jtag_command *cmd)
{
	...
	if (ck->pending_ir_cmd) {
		...
		for (int i = 0; i < ir_scan->num_fields; i++) {
...
static int cklink_execute_queue(void)
{
...
static int cklink_init(void)
{
	...
	if (jtag_libusb_open(cklink_default_vids, cklink_default_pids,
...
static const char * const cklink_transports[] = { "jtag", NULL };
