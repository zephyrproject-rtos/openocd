...
struct flash_device ene_flash_device =
	FLASH_ID("ISPI flash", 0x03, 0x00, 0x02, 0x20, 0x60, 0x00852013, 0x100, 0x1000, 0x80000);
...
static int eneispi_erase_sector(struct flash_bank *bank, int sector)
{
	...
	sectorsize = bank->sectors[sector].size;
	...
	case 0x10000:
		...
		break;
	case 0x08000:
		cmd = CMD_BLOCK_ERASE_32K;
		break;
	case 0x01000:
		cmd = CMD_BLOCK_ERASE_4K;
		break;
	default:
		cmd = CMD_BLOCK_ERASE_64K;
		break;
	...
	eneispif_write_reg(bank, ISPICMD, CMD_WRITE_ENABLE); /* Write enable */
	retval = eneispif_write_reg(bank, ISPICMD, cmd);    /* Erase 64KB */
	if (retval != ERROR_OK)
		goto done;
			  ...
			  uint32_t count)
{
	...
	while (count > 0) {
		...
		retval = eneispif_write_reg(bank, ISPICMD, (cur_count << 16) | CMD_PAGE_WRITE);
		if (retval != ERROR_OK)
			goto err;
...
static int eneispif_read_flash_id(struct flash_bank *bank, uint32_t *id)
{
	...
	eneispif_write_reg(bank, ISPICFG, conf | 0x01); /* Cmmmand enable */
	retval = eneispif_write_reg(bank, ISPICMD, 0x0004009F);
	if (retval != ERROR_OK)
		goto done;
	...
	retval = eneispif_read_reg(bank, &value, ISPIDAT);
	LOG_DEBUG("ISPDAT = (0x%08" PRIx32 ")", value);
	if (retval != ERROR_OK)
		goto done;

	*id = ((value & 0xFF) << 16) | (value & 0xFF00) | ((value >> 16) & 0xFF);
...
static int eneispif_probe(struct flash_bank *bank)
{
	...
	LOG_INFO("Assuming ISPI as specified at address " TARGET_ADDR_FMT
		 " with ctrl at " TARGET_ADDR_FMT,
		 eneispif_info->ctrl_base, bank->base);
...
static int get_eneispif_info(struct flash_bank *bank, struct command_invocation *cmd)
{
	...
	command_print_sameline(cmd,
			       ...
			       eneispif_info->dev->name, eneispif_info->dev->device_id);
